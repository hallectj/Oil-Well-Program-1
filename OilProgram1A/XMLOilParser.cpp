...
XMLOilParser::XMLOilParser(){
	XMLOilParser(m_sFileName);
}

//-----------------------------------
// Constructor
//-----------------------------------
XMLOilParser::XMLOilParser(string fileName) {
    char line[128];
	m_iNumWells = 0;
	m_iNumSensors = 0;
	m_iNextWellNumber = 0;
...
void XMLOilParser::setFileName(string fileName){
	this->m_sFileName = fileName;
}

void XMLOilParser::createWellInfoFile(){
  char line[128];

  ofstream outFileWell("oilWellInfo.txt");
  inFile = new ifstream();
  ...
  if(inFile->is_open()){
    while(getNextLine(line, 127)){
        if(strcmp(line, "<WELLID>") == 0){
            ...
            outFileWell << "well_id: " << line << endl;
        }else if(strcmp(line, "<OPERATOR>") == 0){
            getNextLine(line, 127);
            outFileWell << "operator: " << line << endl;
...
void XMLOilParser::createSensorInfoFile(){
  char line[128];

  ofstream outFileSensor("oilSensorInfo.txt");
  inFile = new ifstream();
  inFile->open(m_sFileName, fstream::in);
  if(inFile->is_open()){
    while(getNextLine(line, 127)){
...
string XMLOilParser::getFileName(){
	return m_sFileName;
}
...
//							the same number.
//--------------------------------------------------------------------------
bool XMLOilParser::getWellData(char *id, char *opr, int *numSensors)
{
	int wNum = 0;
	char line[128];
	// Reopen the file
...
//----------------------------------------------------------------------------------
bool XMLOilParser::getSensorData(char *wellID, char *type, char *className, 
...
//-----------------------------------
int XMLOilParser::getSensorCount(){
...
//------------------------------------------------
bool XMLOilParser::getNextLine(char *buffer, int n){
    bool    done = false;
	char    tempBuf[128];
	char	*temp;
    while(!done)
	...
	temp = tempBuf;
	while(isspace(static_cast<unsigned char>(*temp))) temp++; // Skip leading white space
